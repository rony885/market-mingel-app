...
const ServiceCategoryEdit = () => {
  ...
  const UpdateCategoryFunc = async (values) => {
    ...
    await axios({
      method: "PUT",
      url: `${process.env.REACT_APP_BASE_URL}/service_api/unpaginate_service_category/${item.id}/`,
      data: formfield,
    })
      .then((response) => {
  ...
  useEffect(() => {
    const updateCategory = async (id) => {
      const { data } = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/service_api/unpaginate_service_category/${id}/`
      );
      setItem(data);
    };
    updateCategory(id);
              ...
              <div className="card">
                <div className="card-header d-flex justify-content-between align-items-center gap-1 mb-3">
                  <h4 className="card-title flex-grow-1 fs-4">
                    <Link to="/">Dashboard</Link> | Edit Service Category
                  </h4>
