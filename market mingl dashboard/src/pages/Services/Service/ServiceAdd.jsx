...
const ServiceAdd = () => {
  ...
  // add
  const AddServiceFunc = async (values) => {
    ...
    await axios({
      method: "POST",
      url: `${process.env.REACT_APP_BASE_URL}/service_api/unpaginate_service/`,
      data: formfield,
    })
      .then((response) => {
        setMessage(response.success, "Service is successfuly created...");
        navigate("/service");
  ...
  const submitAddServiceForm = async (
    values,
    { setErrors, setSubmitting, resetForm }
  ) => {
    try {
      setSubmitting(true);
      await AddServiceFunc(values);
      setSubmitting(false);
      // resetForm();
    } catch (error) {
