...
const EmigrantDivisionDetailsAdd = () => {
  // data fetching
  const {
  ...
  // add
  const AddEmigrantServiceFunc = async (values) => {
    ...
    await axios({
      method: "POST",
      url: `${process.env.REACT_APP_BASE_URL}/emigrantService_api/unpaginate_emigrantService/`,
      data: formfield,
    })
      .then((response) => {
        setMessage(
          response.success,
          "Emigrant service is successfuly created..."
        );
  ...
  const submitAddEmigrantServiceForm = async (
    values,
    { setErrors, setSubmitting, resetForm }
  ) => {
    try {
      setSubmitting(true);
      await AddEmigrantServiceFunc(values);
      setSubmitting(false);
      // resetForm();
    } catch (error) {
            ...
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center gap-1 mb-3">
                <h4 className="card-title flex-grow-1 fs-4">
                  <Link to="/">Dashboard</Link> | Add Emigrant Division
                </h4>
              ...
              <div className="card-body">
                <Formik
                  ...
                  onSubmit={submitAddEmigrantServiceForm}
                  validate={validate}
                >
                  {({
                    handleSubmit,
...
export default EmigrantDivisionDetailsAdd;
